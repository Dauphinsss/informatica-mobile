...
export default function MisPublicacionesScreen() {
  ...
  const handleLike = useCallback(
    async (publicacion: PublicacionConMateria) => {
      if (!user) return;
      try {
  ...
  const handleCommentPress = useCallback(
    (publicacion: PublicacionConMateria) => {
      navigation.navigate("PublicationDetail", {
      ...
      list = list.sort((a, b) => {
        if (a.materiaSemestre !== b.materiaSemestre) {
          return sortOrder === "desc"
            ? b.materiaSemestre - a.materiaSemestre
            : a.materiaSemestre - b.materiaSemestre;
        }
    ...
    <View style={[styles.container]}>
      <Appbar.Header>
        {!selectionMode ? (
          ...
          <Appbar.Content
            title={`${selectedIds.length} seleccionada${selectedIds.length !== 1 ? "s" : ""}`}
          />
        )}
        {selectionMode ? (
          <>
      ...
      <View style={{ flex: 1 }}>
        ...
        {cargando ? (
          <View style={{ flex: 1, paddingTop: 8 }}>{renderSkeletons()}</View>
        ) : filtered.length === 0 ? (
        ...
        <CustomAlert
          ...
          message={
            deletedCount != null
              ? `Se eliminaron ${deletedCount} publicación${deletedCount !== 1 ? "es" : ""} correctamente.`
              : "Se eliminaron las publicaciones correctamente."
          }
          type="info"
          buttons={[
            {
              text: "OK",
              onPress: () => setSuccessDeleteVisible(false),
              mode: "contained",
            },
          ]}
        />
